...
const AppCore = {
  ...
  // ==========================================================
  config: {
    // Backend principal (Azure) con override por localStorage
    backendUrl:
      localStorage.getItem("backendURL") ||
      "https://stock-backend-clean-e7ggeebabdfgeeh4.brazilsouth-01.azurewebsites.net",
...
// ============================================================
AppCore.login = async function (username, password) {
  try {
    const res = await fetch(this.config.backendUrl + "/login", {
      method: "POST",
    ...
    this.auth.set(data.token, data.role);
...
AppCore.apiRequest = async function (path, options = {}) {
...
AppCore.buscar = async function () {
  ...
  try {
    const res = await this.apiRequest("/query", {
      method: "POST",
...
AppCore.cargarCatalogo = async function () {
  ...
  try {
    const res = await this.apiRequest("/catalog");
    if (!res.ok) throw new Error();
